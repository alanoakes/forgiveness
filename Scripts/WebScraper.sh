...
# Data: https://www.blueletterbible.org/lang/lexicon/lexicon.cfm?page=1&strongs=H5375#lexResults

StrongsArray=( H3722 H5375 H5545 H5546 H5547 G0859 G0863 G5483 )

for i in ${StrongsArray[@]}; do
	for j in {1..20}; do

		echo "https://www.blueletterbible.org/lang/lexicon/lexicon.cfm?page=$j&strongs=$i&ss=1#lexResults" |
			wget --no-http-keep-alive --no-cache --no-cookies	--header="Accept: text/html" \
			...
			sed 's/   Unchecked Copy Box //g' >> WebScraperOutput.txt
